...
{
    public delegate void ChangeStateButton();//delegate là ủy quyền gần giống con trỏ hàm
    ...
    {
        public System.Drawing.Bitmap imgError = Properties.Resources.erroricon;//lấy từ thư mục .../Resources vào erroricon
        ...
        public string formatStringToName(string name)//chuẩn hóa chuỗi họ tên
        {
            name = name.ToLower();
        ...
        public bool Check(params ValidateParam[] ps)//params dùng để nhóm các đối số vào một mảng và thao tác trên mảng đó
        {
            //ở đây dữ liệu đầu vào của hàm là nhiều biến có kiểu ValidateParam
            foreach (ValidateParam p in ps)
            {
                switch (p.type)
                {
                    case ValidateType.NULL:
                        p.lb.Visible = true;// hiện nhãn khi chương trình thực hiện,nếu=false thì ẩn
