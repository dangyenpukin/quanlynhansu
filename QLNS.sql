...
GO
CREATE PROC SpLogin---lấy mật khẩu của nhân viên
(
	@MANV VARCHAR(10),
	@MATKHAU VARCHAR(50)	
)
...
BEGIN
	...
	SELECT @@IDENTITY--trả về giá trị cột cuối cùng có thuộc tính indentity
END
GO
...
go
create proc EditCMND(---thay số chứng minh
	@CMNDCU varchar(12),
	@CMND varchar(12),
	@NGAY_CAP datetime,
	@NOI_CAP nvarchar(30),
...
begin
	...
	DECLARE @SOHS INT
	SET @SOHS=CAST(SUBSTRING(@MAHS,3,LEN(@MAHS)) AS INT)+1--cast là ép kiểu,len là tổng kí tự trong chuỗi tính cả khoảng trắng đầu chuỗi,bỏ qua khoảng trắng sau
	--hàm substring là hàm cắt chuỗi ở giữa,từ vị trí 3,lấy đúng LEN(@MAHS) kí tự
	IF @SOHS<10
...
CREATE TRIGGER KT_THAN_NHAN ON QUAN_HE_GIA_DINH--loại bỏ những quang hệ đã trùng của 1 ng
FOR UPDATE,INSERT
AS
BEGIN
	IF UPDATE(HOTEN)
